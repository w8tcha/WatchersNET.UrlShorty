...
namespace WatchersNET.DNN.Modules.UrlShorty
{
    ...
    public partial class Dnn404 : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            var portalKey = string.Format("URLSHORTY#{0}#", this._portalSettings.PortalId);

            var settingsDictionary = Host.GetHostSettingsDictionary();

            if (!settingsDictionary.ContainsKey(string.Format("{0}UseErrorLog", portalKey)))
            {
                return;
            }

            if (string.IsNullOrEmpty(settingsDictionary[string.Format("{0}UseErrorLog", portalKey)]))
            {
                return;
            }
            ...
            if (!bool.TryParse(settingsDictionary[string.Format("{0}UseErrorLog", portalKey)], out result))
            {
                return;
            }
        ...
        private void SetLanguage()
        {
            ...
            {
                sLanguage = this.Request.QueryString["language"];
            }

            this.lErrorHeader.Text = Localization.GetString("lErrorHeader.Text", this.SResXFile, sLanguage);
            this.lErrorMessage.Text = Localization.GetString("lErrorMessage.Text", this.SResXFile, sLanguage);
            this.lReasonInfo.Text = Localization.GetString("lReasonInfo.Text", this.SResXFile, sLanguage);

            HtmlGenericControl ulReasons = new HtmlGenericControl("ul");

            HtmlGenericControl liReason1 = new HtmlGenericControl("li")
                {
                   InnerText = Localization.GetString("liReason1.Text", this.SResXFile, sLanguage) 
                };
            HtmlGenericControl liReason2 = new HtmlGenericControl("li")
                {
                   InnerText = Localization.GetString("liReason2.Text", this.SResXFile, sLanguage) 
                };
            HtmlGenericControl liReason3 = new HtmlGenericControl("li")
                {
                   InnerText = Localization.GetString("liReason3.Text", this.SResXFile, sLanguage) 
                };
            ...
            HtmlGenericControl spanHead = new HtmlGenericControl("span")
                {
                   InnerText = Localization.GetString("spanHead.Text", this.SResXFile, sLanguage) 
                };
            HtmlGenericControl spanFoot = new HtmlGenericControl("span")
                {
                   InnerText = Localization.GetString("spanFoot.Text", this.SResXFile, sLanguage) 
                };

            HtmlAnchor linkMainPage = new HtmlAnchor
                {
                    Title = Localization.GetString("linkMainPage.Text", this.SResXFile, sLanguage), 
                    InnerText = Localization.GetString("linkMainPage.Text", this.SResXFile, sLanguage), 
            ...
            this.lApologize.Text = Localization.GetString("lApologize.Text", this.SResXFile, sLanguage);
